...
export async function generateMetadata(
  ...
  const article = philosophicalWritings.find(p => p.slug === resolvedParams.slug);
  ...
  return {
    title: `${article.title} | Kapil Jain`,
    description: article.excerpt,
  };
}

export default async function PhilosophyPostPage({ params }: Props) {
  const resolvedParams = await params;
  const article = philosophicalWritings.find(p => p.slug === resolvedParams.slug);
  ...
  const filePath = path.join(process.cwd(), 'public', article.contentPath);
  ...
  try {
    ...
    content = "Sorry, the content for this article could not be loaded.";
  }
...
export async function generateStaticParams() {
  return philosophicalWritings.map((article) => ({
    slug: article.slug,
  }));
}
